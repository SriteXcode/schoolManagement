...
const PRINCIPAL_MESSAGE_PREVIEW_WORDS = 25;

const fallbackPrincipalMessage = {
    name: 'Dr. Alaric Vance',
    designation: 'Institutional Principal',
    image: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80',
    message: 'Education is not merely the accumulation of facts, but the training of the mind to think beyond the horizon. We are dedicated to this journey of discovery.'
};

const getWordPreview = (text, wordLimit = PRINCIPAL_MESSAGE_PREVIEW_WORDS) => {
    const words = text.trim().split(/\s+/);
    return {
        preview: words.length > wordLimit ? `${words.slice(0, wordLimit).join(' ')}...` : text,
...
const Home = () => {
    ...
    const [principalMessage, setPrincipalMessage] = useState(fallbackPrincipalMessage);
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            ...
            // Replace this fallback with an API response when a principal message endpoint is available.
            setPrincipalMessage(fallbackPrincipalMessage);
        };
        fetchInitialData();
    }, []);
    ...
    const getUniversalTableData = () => {
        ...
        filteredExams.forEach(e => {
            ...
            classMap[classId].dates[new Date(e.date).toISOString().split('T')[0]] = {
                ...
                maxMarks: e.maxMarks,
                syllabus: e.syllabus,
                classDetails: `${e.sClass.grade}-${e.sClass.section}`,
                date: e.date
            };
        });
    ...
    const principalMessagePreview = getWordPreview(principalMessage.message);

    return (
        <div className="min-h-screen bg-[#F8FAFC] flex flex-col relative overflow-x-hidden">

            {/* Hero Section */}
            <div className=" flex flex-col justify-center items-center text-center px-4 pt-2 pb-12">

                {/* Carousel Section */}
                {carouselItems.length > 0 ? (
                    <div className="w-full max-w-6xl h-[550px] md:h-[600px] lg:h-[480px] relative rounded-[2.5rem] overflow-hidden shadow-2xl mb-14 group border-8 pt-10 border-white">
                            ...
                            >
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent flex flex-col justify-end p-10 md:p-20 text-left">
                                    <motion.div
                                        ...
                                        className="space-y-4"
                                    >
                                        <span className="px-4 py-1.5 bg-indigo-600 text-white rounded-sm text-[10px] font-black uppercase tracking-widest">Featured Insight</span>
                                        <p className="text-white pt-6 text-lg font-bold md:text-6xl font-black tracking-tight leading-none">
                                            {carouselItems[currentIndex].title}
                                        </p>
                                        <p className="text-white/70 pt-0 text-lg md:text-2xl font-semibold max-w-3xl">
                    ...
                    <div className="w-full max-w-6xl h-[500px] md:h-[550px] bg-indigo-50 rounded-[3rem] flex items-center justify-center mb-24 border-2 border-dashed border-indigo-200">
          ...
          className="bg-indigo-50/50 text-indigo-700 px-4 py-1 rounded-sm text-sm font-black mb-6 border border-indigo-100"
        >
           ...
           Smart Campus Ecosystem
                        ...
                        <div className="relative z-10 space-y-6">
                            <div className="flex items-center gap-4">
                                <div className="w-14 h-14 rounded-full bg-indigo-500 overflow-hidden border-2 border-white/20">
                                    <img src={principalMessage.image} alt="Principal" className="w-full h-full object-cover" />
                                </div>
                                <div>
                                    <p className="text-white font-black text-lg leading-none">{principalMessage.name}</p>
                                                            ...
                                                            {tableDates.map(d => (
                                                                <td key={d} className="p-2 border-r border-slate-100 text-center">
                                                                    {row.dates[d] ? (
                                                                        ...
                                                                        >
                                                                            <div className="text-xs font-black text-indigo-600 uppercase tracking-tight group-hover/cell:text-indigo-700">{row.dates[d].subject}</div>
                                                                            <div className="text-[9px] font-bold text-slate-400 mt-1.5 uppercase tracking-wider">{row.dates[d].time}</div>
                                                                            <div className="text-[8px] font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full mt-2 uppercase tracking-widest">{row.dates[d].shift}</div>
                                                                        </button>
                                                                    ) : (
                ...
                <div className="w-full max-w-5xl bg-white rounded-4xl shadow-soft-xl overflow-hidden flex flex-col md:flex-row mx-4 border border-slate-100">
                    <div className="bg-slate-900 p-[2rem_2rem_2rem_2rem] md:p-10 text-white text-left md:w-2/5 relative">
                        <div className="absolute bottom-0 right-0 w-32 h-32 bg-indigo-600/20 rounded-full blur-3xl -mb-16 -mr-16" />
                        <p className="text-3xl font-black mb-4 tracking-tight uppercase">Get In Touch</p>
                        <p className="text-slate-400 mb-6 text-lg font-medium leading-relaxed">Our support team will contact you!</p>
            ...
            <AnimatePresence>
                {showPrincipalMessage && (
                    <motion.div
                        initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[180] bg-slate-900/70 backdrop-blur-md flex items-center justify-center p-6"
                    ...
                    >
                        <motion.div
                            ...
                            className="bg-white w-full max-w-2xl rounded-[12px_0px_12px_0px] shadow-2xl overflow-hidden relative border border-white/20"
                            onClick={e => e.stopPropagation()}
                        >
                            <button
                                onClick={() => setShowPrincipalMessage(false)}
                            ...
                            <div className="p-8 md:p-12 text-left">
                                <div className="flex items-center gap-4 mb-8 pr-12">
                                    <div className="w-14 h-14 rounded-full bg-indigo-500 overflow-hidden border border-slate-100">
                                        <img src={principalMessage.image} alt="Principal" className="w-full h-full object-cover" />
                                    </div>
                                    <div>
                                        <h2 className="text-2xl font-black text-slate-900 leading-none">{principalMessage.name}</h2>
                                        <p className="text-indigo-600 text-[10px] font-black uppercase tracking-widest mt-2">{principalMessage.designation}</p>
                                    </div>
                                </div>

                                <div className="max-h-[55vh] overflow-y-auto pr-2">
                                    <p className="text-slate-600 text-lg font-semibold italic leading-relaxed">
...
<div className="flex justify-start items-center gap-4 mb-4" >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4 bg-slate-50 p-6 rounded-2xl border border-slate-100/50">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Time & Shift</div>
                                            <div className="text-slate-900 font-bold text-xs">{selectedExamDetail.time} ({selectedExamDetail.shift})</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Max Marks</div>
                                            <div className="text-slate-900 font-bold text-xs">{selectedExamDetail.maxMarks} Marks</div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="p-6 bg-indigo-50/30 rounded-xl border border-indigo-100/50 max-h-[180px] overflow-y-auto scrollbar-hide">
                                        <div className="space-y-3">
                                            {selectedExamDetail.syllabus
                                                ...
                                                : <p className="text-slate-400 text-sm italic font-semibold">No syllabus specified.</p>
                                            }
                                        </div>
                                    </div>
                                </div>
...
const QuickLink = ({ to, label, color }) => (
    <Link to={to} className={`${color} p-6 rounded-2xl font-bold text-sm uppercase tracking-widest text-center hover:scale-[1.02] active:scale-95 transition-all shadow-sm flex items-center justify-center border border-white/50 hover:shadow-xl`}>
        {label}
    </Link>
);
