        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            days.push(
                <div
                    ...
                    className={`h-20 sm:h-12 md:h-16 border border-slate-100 p-1 sm:p-2 relative cursor-pointer hover:bg-indigo-50/50 transition-all overflow-hidden group ${new Date().toDateString() === dateString ? 'bg-indigo-50/50' : 'bg-white'
                        }`}
                >
                    ...
                    <div className="mt-0.5 sm:mt-1 space-y-0.5">
                        {allDayItems.slice(0, 3).map((item, idx) => (
                            <div
                                key={idx}
                                className={`text-[7px] sm:text-[9px] px-1 py-0.5 rounded sm:rounded-md truncate text-white font-bold shadow-sm ${item.itemType === 'Exam' ? 'bg-rose-500' :
                    ...
                    <div className="flex gap-6 overflow-x-auto pb-4 pt-4 px-4 snap-x snap-mandatory scrollbar-hide">
                        {upcomingItems.length > 0 ? upcomingItems.map((item, idx) => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="text-left">
                                            ...
                                            <h4 className="font-black text-slate-800 text-xl truncate max-w-[180px] text-left">{item.title || item.subject}</h4>
                                        </div>
                                    </div>
                                ...
                                <p className="text-sm font-semibold text-slate-500 line-clamp-2 text-left mb-4 leading-relaxed min-h-[2.5rem]">
                                ...
                                <div className="flex items-center justify-between pt-6 border-t border-slate-200/50">
                                    ...
                                    <div className="flex gap-3">
                                        <button
                                            onClick={(e) => { e.stopPropagation(); navigateToGallery(item); }}
                                            className="w-10 h-10 rounded-md bg-white text-slate-400 flex items-center justify-center hover:bg-teal-50 hover:text-teal-600 transition-all shadow-sm cursor-pointer"
                                        >
                                            <FaExternalLinkAlt size={12} />
                                        </button>
                                        <button
                                            onClick={(e) => { e.stopPropagation(); handleMoreDetail(item); }}
                                            className="w-10 h-10 rounded-md bg-white text-slate-400 flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-all shadow-sm cursor-pointer"
                                        >
                            ...
                            <div className="w-full py-24 bg-white rounded-[3rem] border border-slate-100 border-dashed text-center">
                                ...
                                <p className="text-slate-400 font-bold italic text-lg">No upcoming agenda found for this period.</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                {showDayModal && selectedDate && (
                    ...
                    >
                        <motion.div
                            ...
                            className="bg-white rounded-xl sm:rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden border border-white/20"
                            onClick={e => e.stopPropagation()}
                        >
                            <div className="p-8 sm:p-12">
                                <div className="flex justify-between items-start mb-2 sm:mb-4">
                                    <div className="text-left pr-8">
                                        <p className="text-xl md:text-2xl font-bold text-slate-900 tracking-tight leading-tight">
                                            {selectedDate.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}
                                        </p>
                                        <div className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.2em] mt-1">Daily Schedule Log</div>
                                    </div>
                                    <button onClick={() => setShowDayModal(false)} className="p-3 bg-slate-100 text-slate-400 rounded-full sm:rounded-full hover:bg-slate-200 transition-colors">
                                        <FaTimes />
                                    </button>
                                </div>

                                <div className="space-y-4 sm:space-y-6 max-h-[70vh] overflow-y-auto pr-2 sm:pr-4 scrollbar-hide custom-scrollbar">
                                    {(() => {
                                        ...
                                        return (
                                            <div className="space-y-4">
                                                {dayEvents.map(e => (
                                                    <div key={e._id} className="group flex flex-col sm:flex-row items-start sm:items-center justify-between p-5 sm:p-6 bg-indigo-50/50 rounded-xl sm:rounded-lg border border-indigo-100/50 hover:bg-white hover:shadow-lg transition-all gap-4">
                                                        <div className="flex items-center gap-4 sm:gap-6 text-left">
                                                            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-lg sm:rounded-lg flex items-center justify-center text-indigo-600 shadow-sm border border-indigo-50 shrink-0">
                                                                <FaBullhorn size={18} />
                                                            </div>
                                                            <div>
                                                                <h4 className="font-black text-slate-900 text-sm sm:text-base">{e.title}</h4>
                                                                <p className="text-[10px] sm:text-xs font-semibold italic text-slate-500 mt-1 h-[20vh] overflow-y-auto scrollbar-hide custom-scrollbar ">{e.description}</p>
                                                                <div className="flex items-center gap-3 mt-3">
                                                                    ...
                                                                    {isEditable && (
                                                            <div className="flex gap-3 group-hover:opacity-100 transition-opacity self-end sm:self-center">
                                                                <button onClick={() => handleEditItem(e)} className="text-[8px] sm:text-[9px] font-black text-teal-600 flex items-center gap-1 hover:underline"><FaEdit size={8} />Edit</button>
                                                                <button onClick={() => handleDeleteItem(e._id)} className="text-[8px] sm:text-[9px] font-black text-teal-600 flex items-center gap-1 hover:underline"><FaTrash size={8} />Delete</button>
                                                            </div>
                                                        )}
                                                                </div>
                                                            </div>
                                ...
                                {isEditable && (
                                    <div className="mt-4 sm:mt-6 flex flex-col sm:flex-row gap-4">
                                        <button
                                            ...
                                            className="flex-1 py-3 bg-indigo-600 text-white rounded-lg font-black uppercase text-[10px] tracking-widest shadow-xl shadow-indigo-100 hover:scale-[1.02] transition-all flex items-center justify-center gap-3"
                                        >
