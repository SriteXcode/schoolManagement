...
const AdmissionCellLayout = () => {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-white md:bg-transparent">
        ...
        <nav className="flex-1 px-4 py-2 space-y-1.5 text-left">
          {isAdmin ? (
            <Link 
              to="/admin/cells" 
              onClick={() => setIsSidebarOpen(false)}
              className="flex items-center space-x-3 py-2 px-4 rounded-lg text-amber-600 bg-amber-50 hover:bg-amber-100 transition-all font-black mb-4 shadow-sm group"
            >
            ...
            <Link 
              ...
              onClick={() => setIsSidebarOpen(false)}
              className="flex items-center space-x-3 py-2 px-4 rounded-lg text-emerald-600 bg-emerald-50 hover:bg-emerald-100 transition-all font-black mb-4 shadow-sm group"
            >
              <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.path}
                onClick={() => setIsSidebarOpen(false)}
                className={`flex items-center space-x-3 py-2.5 px-4 rounded-lg transition-all font-bold ${
                  isActive 
        ...
        <div className="p-4 pt-2 space-y-2">
          <Link to="/" className="flex items-center justify-center p-2 text-slate-400 hover:text-slate-600 font-bold text-sm transition" onClick={() => setIsSidebarOpen(false)}>
             Exit Dashboard
          </Link>
          <button onClick={handleLogout} className="w-full flex items-center justify-center space-x-2 py-3 px-4 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition font-black shadow-sm">
